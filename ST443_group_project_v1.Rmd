...
logistic_predictions <- predict(logistic_model, newdata = test_data1, type = "response")
...
conf_matrix <- confusionMatrix(as.factor(logistic_predicted_labels), as.factor(test_data1$label))
print(conf_matrix)
```
...
qda_predictions <- predict(rda_model, test_data1[,-1])  # Exclude label column
predicted_labels_qda <- qda_predictions$class
```

```{r}
# Confusion matrix
conf_matrix <- confusionMatrix(predicted_labels_qda, as.factor(test_data1$label))
conf_matrix
```
