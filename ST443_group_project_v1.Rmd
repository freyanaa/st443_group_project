...
gene_stats <- data.frame(
  ...
  # Subset for CD4+T cells (assuming label column is named `label`)
...
# Select 4 genes at random (excluding the label column)
...
# Combine the two scatter plots into one
combined_scatter_plots <- scatter_plot1 + scatter_plot2  # Combine plots with patchwork
...
# AUC Calculation
knn_probabilities <- knn(train_X, test_X, train_Y, k = optimal_k, prob = TRUE)  # Use the prob argument
...
# T1.3 Optimizing Classification Performance through Advanced Techniques
...
The F1 score of 0.947 achieved with Logistic Regression using L2 Regularization demonstrates a significant improvement from the initial F1 score of 0.711, highlighting the effectiveness of regularization in enhancing model performance. This improvement reflects the model's strong ability to balance false positives and false negatives. Although slightly lower than advanced approaches like Elastic Net or the meta-classifier, this result still showcases a highly effective classification model. The substantial gain in F1 score underscores the value of L2 regularization in creating a robust and reliable classifier.
...
# Extract metrics
accuracy_elastic_net <- conf_matrix_regularization$overall["Accuracy"]
balanced_accuracy_elastic_net <- conf_matrix_regularization$byClass["Balanced Accuracy"]
f1_score_elastic_net <- conf_matrix_regularization$byClass["F1"]
```
...
cat("Accuracy:", round(accuracy_elastic_net, 3), "\n")
cat("Balanced Accuracy:", round(balanced_accuracy_elastic_net, 3), "\n")
cat("F1 Score:", round(f1_score_elastic_net, 3), "\n")
cat("AUC:", round(auc_elastic_net, 3), "\n")
```
