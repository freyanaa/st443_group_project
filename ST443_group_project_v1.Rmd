...
lda_probs <- lda_predictions$posterior[, "TREG"]

# Create ROC curve
roc_curve_lda <- roc(test_data1_numeric, lda_probs)
...
```{r}
# Create copies of the datasets for logistic regression
train_data_log <- train_data1
test_data_log <- test_data1

# Convert 'label' to a binary numeric variable in the new datasets
train_data_log$label <- ifelse(train_data_log$label == "TREG", 1, 0)
test_data_log$label <- ifelse(test_data_log$label == "TREG", 1, 0)

# Verify the changes
table(train_data_log$label)
table(test_data_log$label)
...
logistic_predictions <- predict(logistic_model, newdata = test_data_log, type = "response")
...
conf_matrix <- confusionMatrix(as.factor(logistic_predicted_labels), as.factor(test_data_log$label))
print(conf_matrix)
```
...
accuracy_logistic <- conf_matrix$overall['Accuracy']
balanced_accuracy_logistic <- conf_matrix$byClass['Balanced Accuracy']
f1_score_logistic <- conf_matrix$byClass['F1']
