...
gbm_tuned_model <- train(
  ...
  trControl = GBcontrol,               # Cross-validation settings
  tuneGrid = GBtune_grid,              # Hyperparameter grid
  metric = "BalancedAccuracy",                    # Metric to optimize (ROC is suitable for classification)
  verbose = FALSE                    # Suppress output
)
```
...
for (i in 1:nrow(search_grid)) {
  ...
  cv_results <- xgb.cv(
    ...
    nrounds = 100,
    early_stopping_rounds = 10,
    feval = balanced_accuracy, # Custom metric
    maximize = TRUE,           # Maximize Balanced Accuracy
    verbose = TRUE
  )
...
non_zero_features <- sum(importance$Gain > 0)  # Gain is a common metric
