...
set.seed(123)
lasso_model <- cv.glmnet(as.matrix(train2_data[, -1]), train_data2$label, alpha = 1, family = "binomial")
```
...
balanced_accuracy <- mean(conf_matrix_lasso$byClass["Balanced Accuracy"], na.rm = TRUE)
f1_score <- mean(conf_matrix_lasso$byClass["F1"], na.rm = TRUE)
