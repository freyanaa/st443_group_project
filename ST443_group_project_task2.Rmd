...
```{r}
# Assign weights to classes / labels 
...
The best value for lambda according to our cross-validation is \~ 0.0316, so this is the value we will use to evaluate our model on the test data. Also, we can see that the number of features selected in our best model is 73. In the following plot we can see how the balanced accuracy differs with respect to the different values that we tested for lambda.
...
In this final part, we evaluate how well our chosen model (lambda = 0.0316) performs on the unseen test data. The metric of evaluation is still the balanced accuracy.

```{r}
# Define threshold values
threshold_values <- seq(0.35, 0.5, length.out = 20)

# Initialize dataframe to store results
results <- data.frame(Threshold = threshold_values, BalancedAccuracy = NA)

# Loop over thresholds and calculate balanced accuracy
for (i in seq_along(threshold_values)) {
  threshold <- threshold_values[i]
  
  # Generate predictions based on the threshold
  lasso_preds <- ifelse(lasso_probs[, "Class1"] > threshold, "Class1", "Class0")
  lasso_preds <- factor(lasso_preds, levels = levels(test_Y))
  
  # Calculate confusion matrix
  confusion_matrix <- table(test_Y, lasso_preds)
  
  # Safeguard: Ensure no division by zero if confusion matrix is empty or incorrect
  if (nrow(confusion_matrix) == 2 && ncol(confusion_matrix) == 2) {
  ...
  results$BalancedAccuracy[i] <- balanced_accuracy
}

# Identify the threshold with the best balanced accuracy
best_result <- results[which.max(results$BalancedAccuracy), ]

# Print results
print(results)
cat("Best Threshold:", best_result$Threshold, "\n")
cat("Best Balanced Accuracy:", best_result$BalancedAccuracy, "\n")
```

We get a final balanced accuracy on the test data of 0.79 for the lasso model.
...
For a random forest of classification trees, we usually use a random selection of m= sqrt(p) predictors as split candidates each time a split in a tree is considered. However in our case, after the pre-selection this would mean m = sqrt(100,000) = \~316, which is extremely computationally expensive and when we evaluate our model on the test data set, we would still have p \>\> n. Thus we will apply a pre-selection of features by removing those features with a variance below 0.01 - given the low variance, these features will likel not serve as good predictors for our classification task.
...
sqrt(31,269) still leaves \~173 features to be considered at each split. We will still try if the model works better for a lower number of features, so that we can ensure n \> p when we run the model on the test set, which has a size of n = 160. Thus we define our grid for the hyperparameter m as follows:
...
We can see that 1591 features have been selected. Now we fit our model on the subset of selected features.
...
The balanced accuracy on the testing set is 0.7465.
...
This ensures a clear separation between data used for model building and evaluation.
...
Defines a grid of hyperparameters for Elastic Net:
- alpha: Controls the balance between L1 (sparse selection) and L2 (shrinkage) penalties.
...
svm_rfe <- rfe(SVMtrain_X, train_Y, sizes=c(10, 20, 30, 40, 50), 
               rfeControl=ctrl, method="svmLinear")
print(svm_rfe)
```
Size 30 was selected because it has the highest accuracy at 0.9391. See all the accuracy values above. 
...
```
Above are the 30 features that were selected. 
